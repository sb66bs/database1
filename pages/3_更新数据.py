...
if st.session_state.show_update_form:
    ...
    if st.button("确认"):
        try:
            conn = get_connection()
            cursor = conn.cursor()
            update_sql = "UPDATE user SET Name=%s,Department=%s,Contact=%s WHERE UserID=%s "
            cursor.execute(update_sql, (user_name, user_department, user_contact, user_id))
            conn.commit()
...
if st.session_state.show_update_device_form:
    ...
    if st.button("确认"):
        try:
            conn = get_connection()
            cursor = conn.cursor()
            update_sql = "UPDATE device SET DeviceName=%s, Model=%s, PurchaseDate=%s, PurchaseCost=%s, Status=%s WHERE DeviceID=%s"
            cursor.execute(update_sql, (device_name, device_model, purchase_date, purchase_cost, device_status, device_id))
            conn.commit()
...
if st.session_state.show_update_use_form:
    ...
    if st.button("确认"):
        try:
            conn = get_connection()
            cursor = conn.cursor()
            update_sql = "UPDATE devicescrap SET DeviceID=%s, UserID=%s, UsageDate=%s, Status=%s WHERE UsageID=%s"
            cursor.execute(update_sql, (device_id, user_id, usage_date, status, usage_id))
            conn.commit()
...
if st.session_state.show_repair_form:
    ...
    if st.button("确认"):
        try:
            conn = get_connection()
            cursor = conn.cursor()
            update_sql = "UPDATE devicerepair SET  DeviceID=%s,RepairDate=%s, RepairContent=%s,RepairCost=%s WHERE RepairID=%s"
...
if st.session_state.show_update_scrap_form:
    ...
    if st.button("确认"):
        try:
            conn = get_connection()
            cursor = conn.cursor()
            update_sql =" UPDATE devicescrap SET DeviceID=%s, ScrapDate=%s, ScrapReason=%s WHERE ScrapID=%s"
            cursor.execute(update_sql, (device_id, scrap_date, scrap_reason, scrap_id))
            conn.commit()
